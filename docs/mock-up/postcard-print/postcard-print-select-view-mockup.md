# はがき宛名面 印刷対象選択モック（v1）

- **画面 ID**: `PRT001`
- **画面名**: 印刷対象選択
- **設計書**: [postcard-address-print-v1-design.md](../../design/postcard-address-print-v1-design.md)

---

## 1. 画面レイアウト

住所録一覧（`ADDR001`）に「印刷」入口を追加するか、専用画面として `/print/select` を用意する。

```
+------------------------------------------------------------------+
| 印刷対象の選択                                    選択: 11 / 200 |
+------------------------------------------------------------------+
| [検索...]  （既存一覧と同型）                                      |
+------------------------------------------------------------------+
| ☐ | 氏名+敬称      | 住所           | 差出人（紐づき）  | 状態   |
| ☑ | 山田 太郎 様   | 東京都...      | 自宅            | OK     |
| ☐ | 佐藤 花子 様   | 大阪府...      | （未紐づけ）     | 除外   |
| ☐ | 鈴木 一郎 様   | ...            | 実家            | OK     |
+------------------------------------------------------------------+
| ⚠ 1 件は差出人未紐づけ／archived のため印刷対象外です。          |
|   [住所録で差出人を紐づけてから再選択 →]                          |
+------------------------------------------------------------------+
|                              [キャンセル]  [プレビューへ進む →]   |
+------------------------------------------------------------------+
```

---

## 2. 操作

| 操作 | 挙動 |
|------|------|
| 行チェック | 複数選択（最大 200 件）。**状態「除外」行はチェック不可** |
| 差出人列 | 読み取り専用。`SenderAddressLink` の有無を表示 |
| 状態「除外」 | 紐づき無し / 差出人 archived → **チェック不可**（選択カウンタにも含めない） |
| プレビューへ進む | 有効 1 件以上必須。`resolve_print_job_items` を呼び出し。**AddressEntry archived / not found が混入していればコマンド失敗**（PRT003 へ進まない） |
| 除外アラート | `resolve_print_job_items` の除外理由に応じ表示（未紐づけ / 差出人 archived）。住所録編集（差出人リンク）への導線 1 行 |
| キャンセル | `printJobDraft` を破棄して離脱 |

---

## 3. ジョブ状態

- 選択 `addressEntryId[]` のみを `sessionStorage.printJobDraft` に保存（`printJobId` / `postcardType` は**含めない**）
- 種別の復元は別キー `printPostcardType`（PRT003 側。本画面では触らない）
- ページリフレッシュ時は本画面へ戻す（state 喪失防止）
- archived `AddressEntry` は一覧に表示しない（既存一覧方針）
- **入場時 prune**: 表示中の検索・ページ `items` は使わない。`filter_active_address_entry_ids(draft.ids)` で archived / not found のみ除外。「N 件は削除またはアーカイブ済みのため選択から外しました」。選択カウンタを同期。一覧未取得・取得失敗では draft を触らない
- **resolve 失敗時（方針 B）**: エラーの offending ID を draft から除外し本画面に残留。残りで再実行可

---

## 4. 遷移

- 有効件数 0 → 進めない
- `resolve` が AddressEntry archived / not found で失敗 → エラー表示 + draft から該当 ID 除外。PRT001 残留（PRT003 へ進まない）
- 有効件数 ≥ 1 かつ resolve 成功 → PRT002（省略可）→ PRT003
