...
`SKILL.md` がワークフローとルールを担う。本ファイルは Step 5（調査・設計書作成）と Step 6–7（自己レビュー）の参照資料のみ。
...
Step 6–7 で使用する checklist。**ブロッカー** がすべて `[x]` になるまで Step 5 に戻る（最大 3 周、SKILL.md 参照）。

```
- [ ] Issue 要件の充足（ブロッカー）
- [ ] 機能矛盾なし（ブロッカー）
- [ ] 実装済み機能との整合（ブロッカー）
- [ ] 方針・要件・アーキテクチャとの整合（ブロッカー）
- [ ] 改善点は対応済み、または未決事項に移した（ループ阻止にしない）
```

| 観点 | 分類 | 確認内容 |
|------|------|----------|
| Issue 要件の充足 | ブロッカー | 受け入れ条件が設計でカバーされているか。漏れ・過剰スコープがないか |
| 機能矛盾 | ブロッカー | 同一機能内・関連機能間で仕様が矛盾していないか |
| 実装整合 | ブロッカー | 既存 API / DB / UI と設計が食い違わないか |
| 方針・要件・アーキテクチャ | ブロッカー | `requirements-and-constraints.md`、Tauri + SQLite、オフライン前提と整合するか |
| 改善点 | 任意 | シンプル化、既存パターン再利用、将来拡張の余地。未対応分は Step 8 サマリの未決事項へ |
